...
const trailingspace = () => EditorView.theme({
    '.cm-trailingspace': {
...
const tabsOrSpaces = () => EditorView.theme({
    '.cm-tab:before': {
        color: '#757575',
        content: "'\\2192'",
        position: 'absolute'
...
const fixedHeightEditor = (height: number) => EditorView.theme({
    '&': {'height': `${height}px`},
    '.cm-scroller': {'overflow': 'auto'},
});

export const extensions = (height: number, prefs?: EditPreferencesInfo, fileType?: string, fileContent?: string) => {
    ...
    if (prefs.syntax_highlighting && mode(fileType)) {
        codeExtensions.push(
            mode(fileType) as Extension,
